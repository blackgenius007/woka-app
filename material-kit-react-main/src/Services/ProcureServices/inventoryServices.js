...
const getAllInventory = async (email, projectName) => {
...
const createInventory = async (id, projectName, inventoryData) => {
  const response = await axios.post(constructURL(`${API_URL_CREATE_INVENTORY}/${id}/${projectName}`), inventoryData);
  return response.data;
};
...
const getTotalInventory = async (email, projectName) => {
  const response = await axios.get(constructURL(`${API_URL_TOTAL_INVENTORY}/${email}/${projectName}`));
  return response.data;
};
...
const outGoingStock = async (email, id, num, quantity, order) => {
  const response = await axios.get(constructURL(`${API_URL_OUTGOING_STOCK}/${email}/${id}/${num}/${quantity}/${order}`));
  return response.data;
};

// Incoming stock
const incomingStock = async (email, id, num, quantity) => {
  const response = await axios.get(constructURL(`${API_URL_INCOMING_STOCK}/${email}/${id}/${num}/${quantity}`));
  return response.data;
};

// Get stock balance
const getStockBalance = async (email, projectName) => {
  const response = await axios.get(constructURL(`${API_URL_STOCK_BALANCE}/${email}/${projectName}`));
  return response.data;
};

// Update inventory item
const updateInventory = async (id, inventoryData) => {
  const response = await axios.post(constructURL(`${API_URL_UPDATE_INVENTORY}/${id}`), inventoryData);
  return response.data;
};
